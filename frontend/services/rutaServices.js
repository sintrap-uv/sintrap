...
export async function getAsignacionesRuta(rutaId) {
  try {
    ...
    const turnosIds = [
      ...new Set(horarios.map((h) => h.tipo_turno_id).filter(Boolean)),
    ];
    ...
    const vehiculosIds = [
      ...new Set(horarios.map((h) => h.vehiculo_id).filter(Boolean)),
    ];
    ...
    if (vehiculosIds.length > 0) {
      const { data: vehiculos } = await supabase
        .from("vehiculos")
        .select(
          "*, tipo_vehiculo:tipo_vehiculo_id(nombre, capacidad_max), conductor:conductor_id(id, nombre)",
        )
        .in("id", vehiculosIds);
    ...
    const usuariosIds = [
      ...new Set(usuarios.map((u) => u.usuario_id).filter(Boolean)),
    ];
...
export async function guardarAsignaciones(rutaId, asignaciones) {
  try {
    ...
    if (
      asignaciones.vehiculosPorTurno &&
      asignaciones.vehiculosPorTurno.length > 0
    ) {
      const turnosIds = [
        ...new Set(
          asignaciones.vehiculosPorTurno.map((item) => parseInt(item.turnoId)),
        ),
      ];
...
export const obtenerRutaActualUsuario = async (userId) => {
  ...
  try {
    const { data, error } = await supabase.rpc(
      "obtener_ruta_usuario_con_geojson",
      {
        p_usuario_id: userId,
      },
    );
...
/**
 * Obtiene una ruta completa con paradas y trayecto GeoJSON por su ID.
 * Se usa para mostrar la ruta más cercana en el mapa.
 *
 * @param {number} rutaId - ID de la ruta
 * @returns {Object|null} Objeto con estructura similar a obtenerRutaActualUsuario()
 */
export const obtenerRutaCompletoId = async (rutaId) => {
  if (!rutaId) return null;

  try {
    // Obtener ruta con trayecto GeoJSON
    ...
    if (rutaError) {
      console.error("Error obteniendo ruta:", rutaError);
      return null;
    }

    // Parsear trayecto GeoJSON si es string
    let trayectoParsado = null;
    try {
      if (typeof rutaData.trayecto === "string") {
        trayectoParsado = JSON.parse(rutaData.trayecto);
      } else {
        trayectoParsado = rutaData.trayecto;
      }
    } catch (e) {
      console.warn("Error parseando trayecto:", e);
    }
    ...
    const paradasData = await obtenerParadasRuta(rutaId);
    ...
    return {
      ...
      rutas: {
        id: rutaData.id,
        numero_ruta: rutaData.numero_ruta,
        nombre: rutaData.nombre,
        trayecto: trayectoParsado,
        color: rutaData.color,
        activa: rutaData.activa,
        updated_at: rutaData.updated_at,
      },
      paradas: paradasData,
    };
  } catch (error) {
    console.error("Exception en obtenerRutaCompletoId:", error);
    return null;
  }
};
